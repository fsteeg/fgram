...
public class TermModel implements Serializable,TreeModel{
	...
	public void deleteChildren(TermModel[] less){
		...
		for(int i=0;i<newChildren.length;i++)
			newChildren[i]=(TermModel)newTemp.elementAt(i);
		setChildren(newChildren);
	}
	...
	public static TermModel makeList(TermModel[] terms){
		if (terms==null) return new TermModel(".",true);
                else {
                    if (terms.length == 0) return new TermModel(".",true);
	...
	public static TermModel makeList(Vector terms){
		if (terms==null) return new TermModel(".",true);
                    ...
                    if (terms.size() == 0) return new TermModel(".",true);
                    else return makeList(0,terms);
                }
	}
	...
	public String toString(PrologEngine engine){
		return toString(engine.getImplementationPeer().getOperators());
	}
