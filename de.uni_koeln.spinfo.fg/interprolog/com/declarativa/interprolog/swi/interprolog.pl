...
ipShowTerminals(L,L) :- ipIsDebugging, !, write('Current grammar terminals:'), write(L), nl.
ipShowTerminals(L,L).
...
handleDeterministicGoal(Goal,RVars,Error,Timestamp,NewBytes)  :- 
	...
	(supportsExceptions ->
		( catch(Goal2,PE,buildTermModel(PE,Error)) -> 
			(var(Error) -> Succeeded=1, NewRVars=RVars2 ; Succeeded=0, NewRVars=[])
			; 
			Succeeded=0, NewRVars=[]) 
		;
		( call(Goal2) -> Succeeded=1, NewRVars=RVars2 ; Succeeded=0, NewRVars=[])
	),
	ipProgressMessage(Goal2-Succeeded/PE),
...
specifyPrologResult(Timestamp,Succeeded,RVars,Result,Error) :- var(Error),
	is_list(RVars),
	\+ ((RVars=[X|_], is_list(X))), % guard against common programmer error
	!,
...
streamContents(C,Handles,Bytes0,Bytes) :-
	(var(Bytes0) -> 
		Job=generated(N), ipProgressMessage(grammar_generating) 
...
newString(S,H1,Hn) --> tC_LONGSTRING, 
