...
*/
public abstract class SubprocessEngine extends PrologEngine{
	...
	public SubprocessEngine(String prologCommand, boolean debug, boolean loadFromJar) {
            ...
            try {
                RecognizerListener availableSetter = new RecognizerListener (){
                    public void recognized(Recognizer source,Object extra){
                        available=true;
                        if (isDebug()) progressMessage("I'm available!");
                    }
                };
            	...
            	tempPath = peer.unescapedFilePath(tempPath);
                command("asserta(library_directory('"+tempPath+"'))");
                progressMessage("Loading initial files...");
                String F = peer.interprologFilename();
                if (loadFromJar) consultFromPackage(F,this);
                else consultRelative(F,this);
	            ...
	            setupCallbackServer();
                //progressMessage("here...");
	            waitUntilAvailable();
                //progressMessage("there...");
	            if(!isWindowsOS()){
                	prepareInterrupt(myHost);
                }
